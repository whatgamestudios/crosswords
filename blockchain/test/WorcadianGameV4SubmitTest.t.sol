...
contract WorcadianGameV4SubmitTest is Test {
    ...
    function test_SubmitBoard_NotPassport_Reverts() public {
        vm.prank(stranger);
        vm.expectRevert(
            abi.encodeWithSelector(WorcadianGameV4.NotPassportWallet.selector, stranger)
        );
    ...
    function test_SubmitBoard_ScoreNotCompetitive_EmitsEvent() public {
        ...
        emit WorcadianGameV4.ScoreNotCompetitive(SCORE_NOT_IN_DICT, SCORE_IN_DICT);
    ...
    function test_SubmitBoard_BoardTooShort_Reverts() public {
        ...
        vm.prank(address(passportWallet));
        vm.expectRevert(
            abi.encodeWithSelector(WorcadianGameV4.InvalidBoardSize.selector, uint256(5))
        );
    ...
    function test_SubmitBoard_BoardTooLong_Reverts() public {
        ...
        vm.prank(address(passportWallet));
        vm.expectRevert(
            abi.encodeWithSelector(WorcadianGameV4.InvalidBoardSize.selector, uint256(122))
        );
    ...
    function test_SubmitBoard_SeedWordNotFound_Reverts() public {
        ...
        vm.prank(address(passportWallet));
        vm.expectRevert(
            abi.encodeWithSelector(WorcadianGameV4.SeedWordNotFound.selector, "FROST")
        );
        game.submitBoard(GAME_DAY, SCORE_IN_DICT, _onlyBoard());
    }
    ...
    function test_SubmitBoard_SeedWordEmptyString_Reverts() public {
        ...
        vm.prank(address(passportWallet));
        vm.expectRevert(
            abi.encodeWithSelector(WorcadianGameV4.SeedWordNotFound.selector, "")
        );
        game.submitBoard(GAME_DAY, SCORE_IN_DICT, _onlyBoard());
    }
    ...
    function test_SubmitBoard_SeedWordAtWrongColumn_Reverts() public {
        ...
        vm.prank(address(passportWallet));
        vm.expectRevert(
            abi.encodeWithSelector(WorcadianGameV4.SeedWordNotFound.selector, "ONLY")
        );
    ...
    function test_SubmitBoard_ValidBoard_EmitsBoardSubmitted() public {
        ...
        emit WorcadianGameV4.BoardSubmitted(GAME_DAY, SCORE_IN_DICT, address(passportWallet));
    ...
    function test_SubmitBoard_ValidBoard_StoresPlayerAndBoard() public {
        ...
        WorcadianGameV4.Submission[] memory subs =
    ...
    function test_SubmitBoard_ScoreMismatch_EmitsEvent() public {
        ...
        vm.expectEmit(false, false, false, true, address(game));
        emit WorcadianGameV4.ScoreMismatch(wrongScore, SCORE_IN_DICT);
    ...
    function test_GetBestSubmissions_BeforeAnySubmission_ReturnsEmpty() public view {
        WorcadianGameV4.Submission[] memory subs = game.getBestSubmissions(GAME_DAY);
    ...
    function test_GetBestSubmissions_UpdatesAfterBetterSubmission() public {
        ...
        WorcadianGameV4.Submission[] memory best = game.getBestSubmissions(GAME_DAY);
    ...
    function test_GetSubmissions_Pagination() public {
        ...
        WorcadianGameV4.Submission[] memory page1 =
            game.getSubmissions(GAME_DAY, SCORE_IN_DICT, 0, 2);
        WorcadianGameV4.Submission[] memory page2 =
    ...
    function test_SubmitBoard_SomeInDictSomeNot() public {
        ...
        (uint256 score, string[] memory words, bool[] memory inDict) = game.analyseBoard(board);
        assertEq(score, expectedScore, "Analyse: score");
        assertEq(words.length, 4, "words len");
        assertEq(words[0], "ONLY", "first word");
        assertEq(words[1], "XO", "second word");
        assertEq(words[2], "SKY", "third word");
        assertEq(words[3], "AX", "fourth word");
        assertEq(inDict.length, 4, "in dict length");
        assertEq(inDict[0], true, "in dict 0");
        assertEq(inDict[1], false, "in dict 1");
        assertEq(inDict[2], true, "in dict 2");
        assertEq(inDict[3], true, "in dict 3");

        vm.prank(address(passportWallet));
        game.submitBoard(GAME_DAY, expectedScore, board);
        assertEq(game.bestScoreByDay(GAME_DAY), expectedScore, "Best day: score");
    }
